...
export default function PlansClient({ currentSubscription, userId, currentUser }: Props) {
    ...
    const [selectedPlanId, setSelectedPlanId] = useState<string>(PLANS_DATA[2].id); // Default to group_session as per old code
    ...
    const handlePay = async (isTrial: boolean = false) => {
        ...
        setLoading(true);
        try {
            // LOGIC PRESERVATION: Using Test Mode Bypass as per "No Logic Change" instruction
