...
export default async function StudentGroupPage() {
    ...
    const [{ data: enrollments }, { data: activeSubs }] = await Promise.all([
        ...
        admin
            .from('subscriptions')
            .select('id, plan_type, metadata, is_trial, start_date, end_date, duration_months')
            .eq('student_id', user.id)
            .eq('status', 'active')
    ...
    if (activeBatch) {
        ...
        return (
            <StudentGroupHub
                ...
                durationMonths={(enrollment?.subscriptions as any)?.duration_months || (activeSub as any)?.duration_months || null}
            />
        );
    }
