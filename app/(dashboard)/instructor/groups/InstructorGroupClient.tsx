...
import { getGroupJourneyLength } from '@/components/ui/journey-progress';
...
export function InstructorGroupClient({ currentUser, initialBatches, initialBatchResources, instructors, waitingQueue }: InstructorGroupClientProps) {
   ...
   const studentStartDate = selectedStudent
      ...
      : null;
   const studentJourneyLength = getGroupJourneyLength(selectedStudent?.subscription?.duration_months);
   const currentDayNumber = studentStartDate
      ? Math.min(studentJourneyLength, Math.max(1, Math.floor((Date.now() - new Date(studentStartDate).getTime()) / 86400000) + 1))
      : 1;
                     ...
                     <div className="flex items-center gap-3 pl-6 border-l border-[#FF8A75]/10 animate-in fade-in slide-in-from-left-4 duration-500">
                        ...
                        <div className={cn("flex items-center gap-2 transition-opacity duration-300", !selectedStudent && "opacity-30 grayscale pointer-events-none")}>
                           <a
                              ...
                              className="h-9 w-9 rounded-xl bg-[#25D366]/10 hover:bg-[#25D366]/20 flex items-center justify-center transition-all shadow-sm border border-[#25D366]/10"
                           ...
                           <button
                              ...
                              className="h-9 px-4 rounded-xl bg-[#EA4335]/10 hover:bg-[#EA4335]/20 text-[#EA4335] flex items-center gap-2 text-[9px] font-black uppercase tracking-widest transition-all border border-[#EA4335]/10 shadow-sm"
            ...
            <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-300">
               <div className="w-full max-w-md bg-white rounded-3xl p-8 shadow-2xl relative">
                  ...
                  <div className="flex items-center gap-3 mb-6">
                     <div className="h-10 w-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center shadow-sm"><img src="/assets/gmail_icon.png" alt="Gmail" className="w-7 h-7 object-contain" /></div>
                     <div>
