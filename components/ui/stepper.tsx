...
export function StepperSeparator({ className }: React.ComponentProps<'div'>) {
    ...
    return (
        <div
            ...
            data-state={state}
            className={cn(
                'm-0.5 rounded-full bg-gray-200 group-data-[orientation=vertical]/stepper-nav:h-12 group-data-[orientation=vertical]/stepper-nav:w-[3px] group-data-[orientation=horizontal]/stepper-nav:h-[3px] group-data-[orientation=horizontal]/stepper-nav:flex-1 transition-colors duration-300',
                className,
            )}
        />
    );
...
export function StepperDescription({ children, className }: React.ComponentProps<'div'>) {
    ...
    return (
        <div data-slot="stepper-description" data-state={state} className={cn('text-[10px] text-muted-foreground mt-1', className)}>
            {children}
        </div>
    );
}
...
export function StepperNav({ children, className }: React.ComponentProps<'nav'>) {
    ...
    return (
        <nav
            ...
            data-orientation={orientation}
            className={cn(
                'group/stepper-nav inline-flex w-full items-start data-[orientation=horizontal]:flex-row data-[orientation=vertical]:flex-col',
                className,
            )}
        >
            {children}
